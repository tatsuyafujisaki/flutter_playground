...
void main() => runApp(
  const MaterialApp(
    home: Scaffold(body: _MyStatefulsWidget(), backgroundColor: Colors.white),
  ),
);
...
class _MyStatefulWidgetState extends State<_MyStatefulsWidget> {
  ...
  Widget build(BuildContext context) => Center(
    child: IconButton(
      icon: Icon(
        player.state == PlayerState.playing ? Icons.pause : Icons.play_arrow,
      ),
      onPressed: () async {
  ...
  Future<void> configureAudioPlayer(AudioPlayer player, String url) async {
    player
      ...
      ..onPositionChanged
          .map(formatInMSs)
          .distinct()
          .listen((mSs) => debugPrint('👀Position changed: $mSs'))
      ..onPlayerStateChanged.listen(
        (playerState) => debugPrint('👀Player state changed: $playerState'),
      )
      ..onSeekComplete.listen((_) => debugPrint('👀Seek complete'))
      ..onPlayerComplete.listen((_) => debugPrint('👀Player complete'));
